...
//Load all services  -  nếu có tên service trong configs/app sẽ require file đó trong core/ten_service
...
require_once 'core/Rou te.php'; //Load Route class
...
if (!empty($allHelpers)) {
    foreach ($allHelpers as $item) {
        if ($item != '.' && $item != '..' && file_exists('app/helpers/' . $item)) { //Kiểm tra file tồn tại
